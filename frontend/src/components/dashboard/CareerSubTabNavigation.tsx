import { isCareerSubTabAllowed } from '../../config/subscriptionConfig'
...
export default function CareerSubTabNavigation({
  ...
  const isKalaVidyaAllowed = isCareerSubTabAllowed('kala_vidya', currentTier)
      ...
      <button
        ...
        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
          activeSubTab === 'kala_vidya'
            ? 'bg-primary text-white shadow-md shadow-primary/20 scale-[1.02]'
            : !isKalaVidyaAllowed
            ? 'text-on-surface-variant/70 hover:bg-surface-variant/60'
            : 'text-on-surface-variant hover:text-primary hover:bg-surface-variant/60'
        }`}
      >
        <span className="material-symbols-outlined text-base">school</span>
        <span>64 Kalas & Receptivity</span>
        {!isKalaVidyaAllowed && (
